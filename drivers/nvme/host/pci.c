					     ...
					     struct request *req)
{
	...
	sg_init_table(iod->meta_sgt.sgl, blk_rq_integrity_segments(req));
	...
	sg_list = dma_pool_alloc(dev->meta_pool, GFP_ATOMIC, &sgl_dma);
	if (!sg_list)
		goto out_unmap_sg;
						...
						struct request *req)
{
	...
	dma_pool_free(dev->meta_pool, iod->meta_list.sg_list, iod->meta_dma);
	dma_unmap_sgtable(dev->dev, &iod->meta_sgt, rq_dma_dir(req), 0);
	mempool_free(iod->meta_sgt.sgl, dev->iod_meta_mempool);
}
...
static int nvme_setup_prp_pools(struct nvme_dev *dev)
{
	...
	dev->meta_pool = dma_pool_create("meta sgl", dev->dev, 256, 256, 0);
	if (!dev->meta_pool)
		goto destroy_small;
