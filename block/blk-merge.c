		...
		unsigned *segs, unsigned max_bytes)
{
	...
	*segs = nsegs;
	bio->page_gap_bit = ffs(page_gaps);
	return 0;
split:
				...
				struct bio *next, struct bio *prev)
{
	...
	if (next->page_gap_bit)
		gap = 1 << (next->page_gap_bit - 1);
	else
		gap = 0;
				     ...
				     struct request *req, struct request *next)
{
	...
	req->__page_gap |= bio_seg_gap(req->q, next->bio, req->biotail) |
			   blk_rq_page_gaps(next);
		...
		struct bio *bio, unsigned int nr_segs)
{
	...
	req->__page_gap |= bio_seg_gap(req->q, bio, req->biotail);
		...
		struct bio *bio, unsigned int nr_segs)
{
	...
	req->__page_gap |= bio_seg_gap(req->q, req->bio, bio);
	bio->bi_next = req->bio;
	req->bio = bio;
