				    ...
				    const char *buf, size_t count)
{
	...
	if (next < 0) {
		...
		WRITE_ONCE(node_migration[nid], NUMA_NO_NODE);
 ...
 *
 * @returns: node id for next memory node in the demotion path hierarchy from
 * 	     @current_node; NUMA_NO_NODE(-1) if @current_node is terminal or
 * 	     its demotion node is not online.
 */
